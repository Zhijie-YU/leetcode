...
My Python Code:(How silly I am!!)
```
    def best_buy_sell(self, l:list):
        n = len(l)
        if n < 2:
            return "the number of values if too small!!"
        nl = np.zeros(n) # -1 means the lowest and 1 means the highest
